...
export function buildContrastByType(seedHex, type, strength = "medium") {
	...
	switch (type) {
		...
		case "complementary": {
			...
			return mix.map((m) => toHex(tinycolor.mix(a, b, m)));
		}

		case "simultaneous": {
			const sat = Math.min(90, 50 * k);
			const accent1 = toHex(s.clone().saturate(sat).lighten(5));
			const accent2 = toHex(s.clone().spin(180).saturate(sat).lighten(5));
			const n1 = toHex(s.clone().desaturate(70).lighten(40));
			const n2 = toHex(s.clone().desaturate(60).lighten(25));
			const n3 = toHex(s.clone().desaturate(50).lighten(15));
		...
		case "extension": {
			...
			return steps.map((d) => toHex(s.clone().lighten(d)));
		}
